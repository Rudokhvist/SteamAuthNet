...
// This derivative work is based on WebBrowser.cs from https://github.com/JustArchiNET/ArchiSteamFarm/tree/0ce04415
// -------------------------------------------------------------------------------------------------
...
namespace SteamAuthNet {
	public sealed class WebBrowser : IDisposable {
		...
		public HttpClient GenerateDisposableHttpClient(bool extendedTimeout = false) {
...
#endif
				Timeout = TimeSpan.FromSeconds(extendedTimeout ? ExtendedTimeoutMultiplier * MobileAuthenticator.ConnectionTimeout : MobileAuthenticator.ConnectionTimeout)
			};
			...
			result.DefaultRequestHeaders.UserAgent.ParseAdd(nameof(SteamAuthNet) + "/" + typeof(MobileAuthenticator).Assembly.GetName().Version);
